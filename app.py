...
if menu == "Panel de Control":
    ...
    # --- 1. EXTRACCIÓN DE DATOS (Nombres de columna corregidos) ---
    # Cambié 'nombre_completo' por 'nombre' que es lo que tienes en tu JSON de la DB
    ...
    # --- 3. LÓGICA DE "RECOBRO" (PUNTO DE CORTE VIRTUAL) ---
    # Usamos st.session_state para guardar el "offset" sin tocar la base de datos
    ...
    # --- 4. MÉTRICAS PRINCIPALES CON ESTILO APPLE ---
    ...
    with c1:
        ...
        if st.button("🔄 Recobro", key="btn_calle", help="Establecer este monto como punto de inicio (Cero)"):
            st.session_state.offset_calle = capital_en_calle
            st.rerun()

    with c2:
        val_caja = max(0, caja_actual - st.session_state.offset_caja)
        st.markdown(f"<div class='metric-card'><small>🏦 EFECTIVO EN CAJA</small><h2 style='color:#34C759;'>RD$ {val_caja:,.0f}</h2></div>", unsafe_allow_html=True)
        if st.button("🔄 Recobro", key="btn_caja", help="Reiniciar conteo de caja actual"):
            st.session_state.offset_caja = caja_actual
            st.rerun()

    with c3:
        val_gastos = max(0, total_gastado_real - st.session_state.offset_gastos)
        st.markdown(f"<div class='metric-card'><small>📉 GASTOS EFECTUADOS</small><h2 style='color:#FF3B30;'>RD$ {val_gastos:,.0f}</h2></div>", unsafe_allow_html=True)
        if st.button("🔄 Recobro", key="btn_gastos", help="Reiniciar historial visual de gastos"):
            st.session_state.offset_gastos = total_gastado_real
            st.rerun()

    st.markdown("---")

    # --- 5. GRÁFICOS DE INTELIGENCIA DE NEGOCIOS (Plotly) ---
    import pandas as pd
    import plotly.express as px
    ...
    with col_left:
        st.subheader("🏆 Top 5 Clientes con Mayor Deuda")
        if res_c.data:
            # Filtramos solo activos y creamos DataFrame con la columna 'nombre'
            df_deudores = pd.DataFrame([
                {'Cliente': c['cliente']['nombre'], 'Deuda': c['balance_pendiente']} 
    ...
    with col_right:
        ...
        # Comparación: Capital Recuperado vs Pendiente
        monto_total_prestado = sum([c['monto_inicial'] for c in res_c.data]) if res_c.data else 0
        capital_recuperado = max(0, monto_total_prestado - capital_en_calle)
        
        df_pie = pd.DataFrame({
            'Tipo': ['Recuperado', 'Pendiente'],
            'Monto': [capital_recuperado, capital_en_calle]
        })
    ...
    # --- 6. SALUD DEL FLUJO DE CAJA ---
    st.markdown("---")
    st.subheader("📈 Análisis de Sostenibilidad")
    
    sf1, sf2 = st.columns(2)
    with sf1:
        st.info(f"**Compromisos Pendientes:** RD$ {total_compromisos:,.2f}")
        if total_compromisos > 0:
            ratio = caja_actual / total_compromisos
            st.progress(min(ratio / 2, 1.0))
            st.caption(f"Tienes {ratio:.2f}x veces el dinero necesario para cubrir tus gastos próximos.")
    ...
    with sf2:
        if res_p.data:
            ...
            fig_area = px.area(df_hist, x='fecha_pago', y='monto_pagado', title="Recaudación Diaria")
            fig_area.update_traces(line_color='#007AFF', fillcolor='rgba(0, 122, 255, 0.1)')
            fig_area.update_layout(height=200, margin=dict(l=0, r=0, t=30, b=0))
